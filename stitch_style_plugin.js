(function () {
    ...
    /* ================================================================
     * Stitch Premium Style v8.0  —  Multi-screen, Custom UI
     *
    ...
    var LogoEngine = {
        ...
        _getLang: function () {
            var manual = Lampa.Storage.get('stitch_logo_lang', 'auto');
    ...
    function applyLogoStyles(img) {
        var logoH = Lampa.Storage.get('stitch_logo_height', '200px');
    ...
    function initHeroProcessor() {
        if (window.__stitch_hero_bound) return;
        window.__stitch_hero_bound = true;
        ...
        Lampa.Listener.follow('full', function (e) {
            ...
            if (bgUrl && domTitle) {
                render[0].style.setProperty('--stitch-mobile-bg', 'url(' + bgUrl + ')');
            }
    ...
    function initCardProcessor() {
        if (window.__stitch_cards_bound) return;
        window.__stitch_cards_bound = true;

        // ── Suppress auto-focus scaling until user interacts ──
        function enableInteraction() {
            document.body.classList.add('stitch-user-interacted');
        ...
        function tagEdges() {
            ...
            for (var r = 0; r < rows.length; r++) {
                ...
                for (var c = 0; c < cards.length; c++) {
                    cards[c].removeAttribute('data-stitch-edge');
                    cards[c].removeAttribute('data-stitch-single');
                }
                if (cards.length === 1) {
                    cards[0].setAttribute('data-stitch-single', 'true');
                } else {
                    cards[0].setAttribute('data-stitch-edge', 'first');
                    cards[cards.length - 1].setAttribute('data-stitch-edge', 'last');
                }
            }
        }
        ...
        function colorizeRatings() {
            ...
            for (var i = 0; i < badges.length; i++) {
                var el = badges[i];
                if (el.getAttribute('data-stitch-colored')) continue;
                ...
                else color = 'var(--stitch-accent)'; // red
                el.style.setProperty('background', color, 'important');
                el.setAttribute('data-stitch-colored', '1');
            }
        }
    ...
    function injectCSS() {
        var old = document.getElementById('stitch-premium-v8');
        if (old) old.remove();

        var accent = Lampa.Storage.get('stitch_accent_color', '#ec130e');
        var fontFam = Lampa.Storage.get('stitch_font_family', 'Manrope');
        var fontSb = Lampa.Storage.get('stitch_font_size_sidebar', '1.1em');
        var scale = Lampa.Storage.get('stitch_card_scale', '1.35');
        var shift = Lampa.Storage.get('stitch_edge_shift', '20px');
        var logoH = Lampa.Storage.get('stitch_logo_height', '200px');
        var blur = Lampa.Storage.get('stitch_backdrop_blur', '30px');
        var sbWidth = Lampa.Storage.get('stitch_sidebar_width', '280px');
        var sbOpacity = Lampa.Storage.get('stitch_sidebar_opacity', '0.45');
        ...
        function getBorderColor(val) {
            if (val === 'accent') return 'var(--stitch-accent)';
            if (val === 'white') return '#ffffff';
            if (val === 'black') return '#000000';
            return 'transparent';
        }

        var bFocus = getBorderColor(Lampa.Storage.get('stitch_card_border_focus', 'accent'));
        var bIdle = getBorderColor(Lampa.Storage.get('stitch_card_border_idle', 'transparent'));
        var cardRad = Lampa.Storage.get('stitch_card_radius', '4px');

        var menuCustomCSS = '';
        if (sbOpacity !== 'native') {
            menuCustomCSS += 'background: rgba(25, 28, 33, ' + sbOpacity + ') !important; ';
        }
        if (sbWidth !== 'native') {
        ...
        function hexToRgb(h) {
            ...
            return r ? parseInt(r[1], 16) + ',' + parseInt(r[2], 16) + ',' + parseInt(r[3], 16) : '236, 19, 14';
        }

        var ratingSet = Lampa.Storage.get('stitch_rating_set', 'no_kp');
        var ratingCSS = '';
        ...
        var fontImport = '@import url("https://fonts.googleapis.com/css2?family=Manrope:wght@400;600;800&family=Inter:wght@400;500;600&display=swap");';

        var css = `
/* ================================================================
   Stitch Premium Style v8.0 — UI Customization
   ================================================================ */
...
:root {
    ...
    --stitch-accent-rgb: ${accentRgb};
    --stitch-accent-gl: rgba(${accentRgb}, 0.5);
    --stitch-accent-bg: rgba(${accentRgb}, 0.7);
    --stitch-text: #f0f0f0;
    --stitch-font: '${fontFam}', 'Inter', sans-serif;
    --stitch-card-scale: ${scale};
    --stitch-shift: 40%;
    --stitch-edge-nudge: ${shift};
    --stitch-sb-blur: ${blur};
    --stitch-duration: 420ms;
    --stitch-ease: cubic-bezier(0.4, 0, 0.2, 1);
    --stitch-radius: ${cardRad};
    --stitch-card-border-focus: ${bFocus};
    --stitch-card-border-idle: ${bIdle};
    --stitch-shadow-text: 0 2px 10px rgba(0,0,0,0.8);
}
...
.items-line__title {
    ...
    color: var(--stitch-text) !important;
    text-shadow: var(--stitch-shadow-text) !important;
...
.card {
    position: relative !important;
    transition: transform var(--stitch-duration) var(--stitch-ease),
...
.card__view {
    ...
    border: 3px solid var(--stitch-card-border-idle) !important;
    transition: border-color var(--stitch-duration) var(--stitch-ease) !important;
}
...
.card__view::before {
    ...
    box-shadow: 0 0 20px var(--stitch-accent-gl), 0 20px 40px rgba(0,0,0,0.6) !important;
    opacity: 0 !important; /* Hidden by default */
    z-index: -1 !important; /* Sit behind the poster */
    pointer-events: none !important;
    transition: opacity var(--stitch-duration) var(--stitch-ease) !important;
...
body:not(.stitch-user-interacted) .card.focus,
body:not(.stitch-user-interacted) .card.hover {
    transform: translate3d(0, 0, 0) !important;
    z-index: 1 !important;
}

body:not(.stitch-user-interacted) .card.focus .card__view,
body:not(.stitch-user-interacted) .card.hover .card__view {
    border-color: var(--stitch-card-border-idle) !important;
}

body:not(.stitch-user-interacted) .card.focus .card__view::before,
body:not(.stitch-user-interacted) .card.hover .card__view::before {
    opacity: 0 !important;
}

body:not(.stitch-user-interacted) .card.focus ~ .card,
body:not(.stitch-user-interacted) .card.hover ~ .card {
    transform: translate3d(0, 0, 0) !important;
}
...
.card:hover {
    ...
    transform: scale3d(var(--stitch-card-scale), var(--stitch-card-scale), 1) !important;
}
...
.card:hover .card__view {
    border-color: var(--stitch-card-border-focus) !important;
}
...
.card.hover ~ .card,
.card:hover ~ .card {
    transform: translate3d(var(--stitch-shift), 0, 0) !important;
    z-index: 1 !important;
}
...
.card[data-stitch-edge="first"].focus,
.card[data-stitch-edge="first"].hover,
.card[data-stitch-edge="first"]:hover {
    transform-origin: left center !important;
    transform: scale3d(var(--stitch-card-scale), var(--stitch-card-scale), 1)
               translate3d(var(--stitch-edge-nudge), 0, 0) !important;
}

/* First card's neighbors: standard shift + extra 20px to compensate */
.card[data-stitch-edge="first"].focus ~ .card,
.card[data-stitch-edge="first"].hover ~ .card,
.card[data-stitch-edge="first"]:hover ~ .card {
    transform: translate3d(calc(var(--stitch-shift) + var(--stitch-edge-nudge)), 0, 0) !important;
}

/* Last card: right-origin scale + 20px leftward nudge (no clip) */
.card[data-stitch-edge="last"].focus,
.card[data-stitch-edge="last"].hover,
.card[data-stitch-edge="last"]:hover {
    transform-origin: right center !important;
    transform: scale3d(var(--stitch-card-scale), var(--stitch-card-scale), 1)
               translate3d(calc(var(--stitch-edge-nudge) * -1), 0, 0) !important;
}

/* Reduce shift for the last card when a non-edge sibling is focused */
.card.focus ~ .card[data-stitch-edge="last"],
.card.hover ~ .card[data-stitch-edge="last"],
.card:hover ~ .card[data-stitch-edge="last"] {
    transform: translate3d(calc(var(--stitch-shift) * 0.5), 0, 0) !important;
}

/* ── SINGLE CARD: use left-origin (no clip) but NO shift ── */
.card[data-stitch-single="true"].focus,
.card[data-stitch-single="true"].hover,
.card[data-stitch-single="true"]:hover {
    transform-origin: left center !important;
    transform: scale3d(var(--stitch-card-scale), var(--stitch-card-scale), 1) !important;
}
...
.full-start-new::before, .full-start::before {
    ...
    background: linear-gradient(to top, var(--stitch-bg) 0%, rgba(10,13,18,0.85) 35%, transparent 80%) !important;
    opacity: var(--stitch-fog-level, 0.15) !important; z-index: 1 !important; pointer-events: none !important; transition: opacity 0.1s linear !important;
}
...
@media (max-width: 768px) {
    ...
    .full-start-new, .full-start {
        background-image: var(--stitch-mobile-bg) !important;
    ...
    .applecation__overlay, .application__overlay {
        display: block !important;
        background: linear-gradient(to top, var(--stitch-bg) 0%, rgba(10,13,18,0.85) 40%, rgba(10,13,18,0.2) 75%, transparent 100%) !important;
        background-color: transparent !important;
        background-image: linear-gradient(to top, var(--stitch-bg) 0%, rgba(10,13,18,0.85) 40%, rgba(10,13,18,0.2) 75%, transparent 100%) !important;
        box-shadow: none !important;
        position: absolute !important;
        top: 0 !important;
        left: 0 !important;
...
@media (max-width: 576px) {
    ...
    :root {
        --stitch-card-scale: 1.1;
        --stitch-shift: 8%;
        --stitch-duration: 300ms;
    }
    .items-line {
...
@media (min-width: 577px) and (max-width: 1024px) {
    ...
    :root {
        --stitch-card-scale: 1.25;
        --stitch-shift: 18%;
        --stitch-duration: 350ms;
    }
    .items-line {
...
@media (min-width: 1920px) {
    ...
    :root {
        --stitch-card-scale: 1.45;
        --stitch-shift: 30%;
        --stitch-duration: 450ms;
    }
    .items-line {
        padding: 60px 0 !important;
    }
    .card__view {
        border-radius: calc(var(--stitch-radius) * 1.5) !important;
    }
}
`;

        var style = document.createElement('style');
        style.id = 'stitch-premium-v8';
    ...
    function initSettings() {
        ...
        var prm = [
            { name: 'stitch_accent_color', type: 'select', values: { '#ec130e': t('red'), '#2ecc71': t('green'), '#3498db': t('blue'), '#e67e22': t('orange'), '#9b59b6': t('purple'), '#e91e63': t('pink') }, default: '#ec130e', title: t('accent_color') },
            { name: 'stitch_logo_lang', type: 'select', values: { 'auto': t('auto'), 'uk': 'Ukrainian (UK/UA)', 'ru': 'Russian (RU)', 'en': 'English (EN)' }, default: 'auto', title: t('logo_lang') },
            { name: 'stitch_font_family', type: 'select', values: { 'Manrope': 'Manrope', 'Roboto': 'Roboto', 'Open Sans': 'Open Sans', 'Inter': 'Inter' }, default: 'Manrope', title: t('font_family') },
            { name: 'stitch_card_border_focus', type: 'select', values: { 'transparent': t('transparent'), 'accent': t('accent_color'), 'white': t('white') }, default: 'accent', title: t('card_border_focus') },
            { name: 'stitch_card_border_idle', type: 'select', values: { 'transparent': t('transparent'), 'accent': t('accent_color'), 'white': t('white'), 'black': t('black') }, default: 'transparent', title: t('card_border_idle') },
            { name: 'stitch_card_radius', type: 'select', values: { '0px': t('square'), '4px': t('small_rad'), '8px': t('med_rad') + ' (' + t('default') + ')', '12px': t('large_rad'), '16px': t('xl_rad') }, default: '8px', title: t('card_radius') },
            { name: 'stitch_font_size_sidebar', type: 'select', values: { 'native': t('native_off'), '0.9em': t('small'), '1.0em': t('normal'), '1.1em': t('large'), '1.2em': t('xlarge') }, default: '1.1em', title: t('sidebar_font_size') },
            { name: 'stitch_sidebar_width', type: 'select', values: { 'native': t('native_off'), '220px': t('compact'), '280px': t('normal') + ' (' + t('default') + ')', '340px': t('wide'), '400px': t('uwide') }, default: '280px', title: t('sb_width') },
            { name: 'stitch_sidebar_opacity', type: 'select', values: { 'native': t('native_off'), '0.1': t('clear'), '0.45': t('glassy'), '0.75': t('dark_glass'), '0.95': t('solid') }, default: '0.45', title: t('sb_opacity') },
            { name: 'stitch_card_scale', type: 'select', values: { '1.1': '1.10x', '1.25': '1.25x', '1.35': '1.35x (' + t('default') + ')', '1.45': '1.45x' }, default: '1.35', title: t('card_scale') },
            { name: 'stitch_edge_shift', type: 'select', values: { '10px': '10px', '20px': '20px', '30px': '30px' }, default: '20px', title: t('edge_shift') },
            { name: 'stitch_logo_height', type: 'select', values: { '80px': t('micro'), '120px': t('tiny'), '150px': t('small'), '200px': t('medium'), '250px': t('large'), '300px': t('xlarge') }, default: '200px', title: t('logo_height') },
            { name: 'stitch_backdrop_blur', type: 'select', values: { '10px': t('light'), '30px': t('premium'), '50px': t('heavy') }, default: '30px', title: t('sb_blur') },
            { name: 'stitch_rating_set', type: 'select', values: { 'all': t('r_all'), 'no_kp': t('r_no_kp'), 'west': t('r_west'), 'tmdb': t('r_tmdb'), 'none': t('r_none') }, default: 'no_kp', title: t('rating_set') }
        ];
    ...
    function bootstrap() {
        if (window.__stitch_premium_v8) return;
        window.__stitch_premium_v8 = true;
        ...
        if (window.Lampa && Lampa.Storage && Lampa.Storage.listener) {
            Lampa.Storage.listener.follow('change', function (e) {
                if (e.name && e.name.indexOf('stitch_') === 0) {
                    injectCSS();
                }
            });
        }
        ...
        console.log('[Stitch Premium] v8.22 — Ultimate Performance & TV Compatibility');
    }
